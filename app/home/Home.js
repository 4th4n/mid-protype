      ...
      <div key={index}>
        ...
        {order.items.map((item, i) => (
          <div key={i}>
            ...
            <Typography variant="body2">Quantity: {item.quantity}</Typography> {/* Display quantity */}
          </div>
        ))}
        <Typography variant="h6" gutterBottom>
          Total: ₱{order.total}
        </Typography>
        <Button variant="contained" color="secondary" onClick={() => onDelete(order.orderNumber)}>Delete Order</Button>
      </div>
    ))}
  </div>
);
